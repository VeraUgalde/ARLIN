...
def inputmRNA(filename):
    ...
    for line in file:
        if line == "SPOTS_START" + "\n":
            ...
            num = int(line.split("\t")[1].split("_")[1])
            if cell_num >= num:
                break
            else:
                cell_num = num
...
def main():
    ...
    for (mrna_channel, mrna_name) in mrnas:
        ...
        for i in range(len(synap_files)):
            ...
            sheet_name = "DIV" + div + " " + treatment
            print(file_name)
            matched_file = glob.glob(directory + "/Results_" + mrna_channel + "/" + file_name.replace(synap_channel, mrna_channel) + "*outline_spots*" + ".txt")
        ...
        for sheet_name, mrna_counter in mrna_counts.items():
            if sheet_name in ctrl_counts.keys():
                keys = list(mrna_counter.keys()) + list(ctrl_counts[sheet_name].keys())
                if len(keys) != 0:
                    max_num = max(keys)
                    mrna_data = [0 if i not in mrna_counter.keys() else mrna_counter[i] for i in range(max_num+1)]
                    ctrl_data = [0 if i not in ctrl_counts[sheet_name].keys() else ctrl_counts[sheet_name][i] for i in range(max_num+1)]
                    ...
                    sheets[sheet_name].append(df)
                    print(sheet_name + " " + mrna_name)
                    print(df)
                else:
                    print("Problem with counter for " + sheet_name + " " + mrna_channel )
